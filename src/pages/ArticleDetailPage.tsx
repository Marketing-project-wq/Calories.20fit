    ...
    return (
      <div>
        <div style={{ maxWidth: 560, margin: "0 auto", padding: "72px 20px", textAlign: "center" }}>
          <div style={{ marginBottom: 12, color: "var(--text-faint)" }}><Icon name="document" size={44} /></div>
          <h1 style={{ fontFamily: "Barlow Condensed, sans-serif", fontSize: 30, textTransform: "uppercase", color: "var(--text)", margin: "0 0 16px" }}>{a.notFound}</h1>
          <Link href={ROUTES.ARTICLES} className="sc-btn-primary" style={{ display: "inline-block", background: "var(--brand)", color: "var(--on-brand)", borderRadius: 12, padding: "11px 20px", fontSize: 14, fontWeight: 700 }}>
            {a.backToList}
          </Link>
        </div>
  ...
  return (
    <div style={{ background: "var(--surface)" }}>
      ...
      <article style={{ maxWidth: 720, margin: "0 auto", padding: "24px 20px 8px" }}>
        <Link href={ROUTES.ARTICLES} style={{ fontSize: 13, fontWeight: 700, color: "var(--brand)" }} className="sc-link-btn">
          ← {a.backToList}
        </Link>
        ...
        <div style={{ margin: "18px 0 10px" }}>
          ...
          {art.isPremium && (
            <span style={{ marginLeft: 10, fontSize: 11, color: "var(--text-faint)", display: "inline-flex", alignItems: "center", gap: 4 }}>
              <Icon name="lock" size={11} /> {a.premium}
            </span>
          )}
        </div>
        <h1 style={{ fontFamily: "Barlow Condensed, sans-serif", fontWeight: 900, fontSize: "clamp(28px,5vw,42px)", lineHeight: 1.05, textTransform: "uppercase", color: "var(--text)", margin: "0 0 14px" }}>
          {art.title[lang]}
        </h1>
        <div style={{ display: "flex", flexWrap: "wrap", gap: "4px 12px", fontSize: 13, color: "var(--text-subtle)", marginBottom: 20 }}>
        ...
        <div style={{ position: "relative" }}>
          ...
          {!showFull && (
            <div style={{ position: "relative", marginTop: -80 }}>
              <div style={{ height: 80, background: "linear-gradient(180deg, rgba(255,255,255,0) 0%, var(--surface) 92%)" }} />
              <div style={{ border: `1px solid ${NUTRI.GREEN_TINT}`, borderRadius: 18, overflow: "hidden", boxShadow: "0 16px 40px -22px rgba(20,20,20,0.2)" }}>
                ...
                <div style={{ padding: "26px 24px", textAlign: "center" }}>
                  <div style={{ marginBottom: 10, color: "var(--brand)", display: "flex", justifyContent: "center" }}><Icon name="lock" size={30} /></div>
                  <h3 style={{ fontFamily: "Barlow Condensed, sans-serif", fontSize: 26, textTransform: "uppercase", color: "var(--text)", margin: "0 0 8px" }}>{a.gateTitle}</h3>
                  <p style={{ fontSize: 14, color: "var(--text-muted)", lineHeight: 1.6, margin: "0 auto 18px", maxWidth: 420 }}>{a.gateSub}</p>
                  <div style={{ display: "flex", flexDirection: "column", gap: 10, maxWidth: 320, margin: "0 auto" }}>
                    <a href={URLS.SIGN_UP} className="sc-btn-primary" style={{ background: "var(--brand)", color: "var(--on-brand)", borderRadius: 12, padding: "13px", fontSize: 15, fontWeight: 700, textDecoration: "none" }}>
                      {cc(lang).gate.signUp} →
                    </a>
                    <a href={URLS.LOGIN} style={{ color: "var(--text)", fontSize: 14, fontWeight: 600, textDecoration: "none" }}>
                      {cc(lang).gate.login}
                    </a>
                  </div>
                </div>
        ...
        {showFull && (
          <>
            <div style={{ marginTop: 34, borderTop: `1px solid var(--border)`, paddingTop: 20 }}>
              <h3 style={{ fontFamily: "Barlow Condensed, sans-serif", fontSize: 20, textTransform: "uppercase", color: "var(--text)", margin: "0 0 10px", display: "flex", alignItems: "center", gap: 8 }}>
                <Icon name="book" size={18} /> {a.sourcesTitle}
              </h3>
              <ul style={{ margin: 0, paddingLeft: 20, display: "flex", flexDirection: "column", gap: 6 }}>
                {art.sources[lang].map((s) => (
                  <li key={s} style={{ fontSize: 13, color: "var(--text-muted)", lineHeight: 1.5 }}>{s}</li>
                ))}
              </ul>
            </div>
        ...
        {related.length > 0 && (
          <div style={{ marginTop: 40 }}>
            <h3 style={{ fontFamily: "Barlow Condensed, sans-serif", fontSize: 22, textTransform: "uppercase", color: "var(--text)", margin: "0 0 14px" }}>{a.relatedTitle}</h3>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(210px,1fr))", gap: 14 }}>
              {related.map((r) => (
                <Link key={r.slug} href={ROUTES.article(r.slug)} className="sc-card" style={{ display: "block", background: "var(--surface)", border: "1px solid var(--border)", borderRadius: 14, padding: "14px 16px", textDecoration: "none" }}>
                  ...
                  <h4 style={{ fontFamily: "Manrope, sans-serif", fontSize: 14.5, fontWeight: 700, color: "var(--text)", lineHeight: 1.3, margin: "6px 0 0" }}>{r.title[lang]}</h4>
                </Link>
              ))}
            </div>
          </div>
