    ...
    <div style={{ background: "#fff" }}>
      ...
      <article style={{ maxWidth: 720, margin: "0 auto", padding: "24px 20px 8px" }}>
        ...
        <div style={{ margin: "18px 0 10px" }}>
          ...
          {art.isPremium && (
            <span style={{ marginLeft: 10, fontSize: 11, color: "#9A9A9A" }}>🔒 {a.premium}</span>
          )}
        </div>
        ...
        <div style={{ display: "flex", flexWrap: "wrap", gap: "4px 12px", fontSize: 13, color: "#8A8A8A", marginBottom: 20 }}>
          ...
          <span>·</span>
          <span>⏱ {a.minRead(art.readTimeMinutes)}</span>
        </div>

        {/* Cover */}
        <div style={{ height: 180, background: `linear-gradient(135deg, ${art.accent}2e, ${art.accent}08)`, borderRadius: 18, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 72, marginBottom: 26 }}>
          {art.coverEmoji}
        </div>
        ...
        {showFull && (
          <>
            <div style={{ marginTop: 34, borderTop: `1px solid #E4E0DB`, paddingTop: 20 }}>
              <h3 style={{ fontFamily: "Barlow Condensed, sans-serif", fontSize: 20, textTransform: "uppercase", color: COLORS.BLACK, margin: "0 0 10px" }}>📚 {a.sourcesTitle}</h3>
            ...
            <p style={{ marginTop: 20, fontSize: 12.5, color: "#8A8A8A", lineHeight: 1.6, borderLeft: `3px solid ${NUTRI.AMBER}`, paddingLeft: 12, background: "#FFFBEB", padding: "12px 14px", borderRadius: "0 10px 10px 0" }}>
              ⚕️ {art.disclaimer}
            </p>
          </>
        )}
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(210px,1fr))", gap: 14 }}>
              {related.map((r) => (
                <Link key={r.slug} href={ROUTES.article(r.slug)} className="sc-card" style={{ display: "block", background: "#fff", border: "1px solid #E4E0DB", borderRadius: 14, padding: "14px 16px", textDecoration: "none" }}>
                  <div style={{ fontSize: 26, marginBottom: 8 }}>{r.coverEmoji}</div>
