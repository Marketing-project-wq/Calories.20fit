  ...
  return (
    <div>
      <div style={{ maxWidth: 640, margin: "0 auto", padding: "24px 18px 8px" }}>
        <h1 style={{ fontFamily: "Barlow Condensed, sans-serif", fontSize: 30, textTransform: "uppercase", color: COLORS.BLACK, margin: "0 0 6px" }}>{mp.pageTitle}</h1>
        <p style={{ fontSize: 14, color: "#6A6A6A", margin: "0 0 16px", lineHeight: 1.55 }}>{mp.sub}</p>
        ...
        {loading && !loaded ? (
          ...
          <>
            {/* Summary bar */}
            <div style={{ display: "flex", flexWrap: "wrap", gap: 10, alignItems: "center", justifyContent: "space-between", background: "#fff", border: `1px solid ${BORDER}`, borderRadius: 14, padding: "14px 16px", marginBottom: 16 }}>
              <div style={{ display: "flex", gap: 20 }}>
                <div>
                  <span style={{ fontSize: 10, color: "#8A8A8A", textTransform: "uppercase", letterSpacing: ".05em", display: "block" }}>{mp.targetLabel}</span>
                  <span style={{ fontFamily: "Barlow Condensed, sans-serif", fontSize: 22, color: COLORS.BLACK, fontVariantNumeric: "tabular-nums" }}>{target.toLocaleString("id-ID")} <span style={{ fontSize: 12, color: "#9A9A9A" }}>{tr.kcal}</span></span>
                </div>
                <div>
                  <span style={{ fontSize: 10, color: "#8A8A8A", textTransform: "uppercase", letterSpacing: ".05em", display: "block" }}>{mp.goalLabel}</span>
              ...
              {plan.meals.map((meal) => (
                <div key={meal.meal} style={{ background: "#fff", border: `1px solid ${BORDER}`, borderRadius: 14, padding: "14px 16px" }}>
                  ...
                  <div style={{ display: "flex", flexDirection: "column" }}>
                    {meal.items.map((it, i) => (
                      <div key={i} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 8, fontSize: 13.5, padding: "7px 0", borderBottom: i < meal.items.length - 1 ? `1px solid #F4F2F0` : "none" }}>
                        <span style={{ flex: 1, color: "#3A3A3A", minWidth: 0 }}>
                          {it.food.name}
                        ...
                        <span style={{ color: COLORS.BLACK, fontWeight: 600, flexShrink: 0, fontVariantNumeric: "tabular-nums" }}>{it.kcal} {tr.kcal}</span>
                      </div>
                    ))}
                  </div>
                </div>
            ...
            <p style={{ fontSize: 12, color: "#9A9A9A", lineHeight: 1.6, margin: "14px 0 0", borderLeft: `3px solid ${NUTRI.GREEN}`, paddingLeft: 10 }}>{mp.note}</p>

            <Link href={ROUTES.TRACKER} className="sc-btn-primary" style={{ display: "inline-block", marginTop: 16, background: COLORS.RED, color: "#fff", borderRadius: 12, padding: "12px 22px", fontSize: 14, fontWeight: 700, textDecoration: "none" }}>
              {mp.addToTracker} →
            </Link>
          </>
        )}
